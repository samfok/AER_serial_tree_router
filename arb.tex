...
We construct the $N\!\_ARB$ as a binary tree recursively (see Figure~\ref{fig:n_arb_h_arb}). 
...
\noindent The recursion ends with $N=1$ or $N=2$ and produces a balanced tree. The top $H\_ARB$'s $P$ port is connected back to itself. 
...
\begin{hse}
*[[ci->\hat{ci}+;[\hat{ci}];[si];so+;co+;[~ci];\hat{ci}-;[~\hat{ci}];co-;[~si];so-
...
*[[ci->\hat{ci}+;[~ci];\hat{ci}-
  \|si->\hat{si}+;[~si];\hat{si}-]]
*[[\hat{ci}->[si];so+;co+;[~\hat{ci}];co-;[~si];so-
  []\hat{si}->so+;[~\hat{si}];so-]]
\end{hse}
...
*[[ci->\hat{ci}+;[~ci];\hat{ci}-
  \|si->\hat{si}+;[~si];\hat{si}-]]
*[[\hat{ci}->[si];so+;[~si];co+;[~\hat{ci}];co-;so-
  []\hat{si}->so+;[~\hat{si}];so-]]
\end{hse}
