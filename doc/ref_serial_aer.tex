...
\begin{easylist}
    ...
    & serial merge excl in (Section~\ref{sec:SERIAL_MERGE})
    && mutually exclusive input (Section~\ref{sec:SERIAL_MERGE_EXCL})
    && non-mutually exclusive input (Section~\ref{sec:SERIAL_MERGE_ARB})
\end{easylist}
...
...
\begin{prs2}
~s(M\-1)M & s(M\-1) & x0 -> y(M\-1)0+
~s(M\-1) -> y(M\-1)0-

~s(M\-1)M & s(M\-1) & x1 -> y(M\-1)1+
~s(M\-1) -> y(M\-1)1-
\end{prs2}
...
\end{prs2}
\begin{prs2}
~__sm(m\+1) & ~_sm & ~_x0 -> ym0+
_sm -> ym0-

~__sm(m\+1) & ~_sm & ~_x1 -> ym1+
_sm -> ym1-
\end{prs2}
...
\begin{prs2}
...
\end{prs2}
\begin{prs2}
~__s(M\-1)M & ~_s(M\-1) & ~_x0 -> y(M\-1)0+
_s(M\-1) -> y(M\-1)0-

~__s(M\-1)M & ~_s(M\-1) & ~_x1 -> y(M\-1)1+
_s(M\-1) -> y(M\-1)1-
\end{prs2}
...
The receiver is responsible for sending spikes to neurons and configuration
data to the neuron and synapse configuration memory.
Sending a spike to the neuron and sending a row of configuration data to the configuration memory

\noindent
For M inputs and 1-of-D encoding,

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Serial merge with mutually exclusive inputs \label{sec:SERIAL_MERGE_EXCL}}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Serial merge with non-mutually exclusive inputs \label{sec:SERIAL_MERGE_ARB}}

This designs adds an arbiter to handle concurrent input, which means we can
support reprogramming the neuron and synapse configuration data on the fly.
...
*[[u0->yo+;[yi];x0o+;[~u0];yo-;[~yi];x0o-
  []u1->yo+;[yi];x1o+;[~u1];yo-;[~yi];x1o-
 ]]
...
The parent requests and grants are handled by a standard n-way arbiter with
