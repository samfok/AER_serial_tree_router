...
\begin{center}
    ...
    $\phi$ & channel has been requested \\
    0 & channel data is 0 \\
    1 & channel data is 1 \\
    $\neg \phi$ & channel no longer requested \\
    \hline
    \end{tabular}
\end{center}
...
\begin{hse}
*[[x`\phi->y`\phi+;[y`e];x`e+
  []x`0->y`0+;[~y`e];x`e-;[~x`0];y`0-;[y`e];x`e+
  []x`1->y`1+;[~y`e];x`e-;[~x`1];y`1-;[y`e];x`e+
  []~x`\phi->y`\phi-;[~y`e];x`e-
 ]]
\end{hse}

\noindent
The source sends the $\phi$ symbol by raising $x_\phi$, and the buffer
acknowledges with $x_e\uparrow$. \\
The buffer relays the $\phi$ to the sink with $y_\phi\uparrow$, and the
sink acknowledges by raising $y_e$. \\
...
and that the sink acknowledges the data by lowering $y_e$.
The second half of the 4-phase handshakes returns the process to the state
ready to receive data. \\
The source sends the $\neg \phi$ symbol by lowering $x_\phi$, and the buffer
acknowledges with $x_e\downarrow$. \\
The buffer relays the $\neg \phi$ to the sink with $y_\phi\downarrow$, and the
sink acknowledges by lowering $y_e$. \\
...
*[[x0&ye->y0+;[~ye];xa+;[~x0];y0-;[ye];xa-
  []x1&ye->y1+;[~ye];xa+;[~x1];y1-;[ye];xa-
 ]]
\end{hse}
