...
\begin{csp}
MUX(b,x)\equiv*[S!(D?)\pll\,T!(R?);S;T!\phi\]\pll*[T!(C?)]
\end{csp}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
In the initial state, \code{$\neg$po$\land\neg$ro$\land\neg$co} is true. With reset circuitry,

\begin{prs2}
~sReset & ~_pi & ~ri->ro+
pReset | _pi->ro-

_sReset & ri & ro->_co-, _po-
~_pReset | ~ri->_co+, _po+
\end{prs2}
...
\begin{tabular}[]{rll}
  ...
  \code{E} & (passive) & communicates with the y (x) address controller \\
\end{tabular} \\ \\
...
\begin{itemize}
\item what is the point of \code{eo}? It's not guarding anything nor is it sent
anywhere. 
\begin{itemize}
\item Indeed \code{eo} is the same wire as \code{vo}, but \code{eo} is used to communicate with the address encoder
\end{itemize}
\end{itemize}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Arbiter (ARB)}

ARB sequences requests from rows or columns. Since these requests could
arrive concurrently, the arbiter needs to be able to arbitrate between 
...
\begin{itemize}
\item \code{L1} (passive) request from left 1 child
\item \code{R} (active) request to parent 
\item \code{A1} (state) arbitration line
\item \code{R1} ?
\item \code{L2} (passive) request from left 2 child
\item \code{R} (active) request to parent 
\item \code{A2} (state) arbitration line
\item \code{R2} ?
\end{itemize}
