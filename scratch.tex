...
\noindent Note that the $\hat{c_i}$ branch of the select statement uses $s_i$ and not $\hat{s_i}$ to ensure that the arbitration will traverse the $s_i$ branch after completing the $c_i$ branch. Note also that $s_o$ is shared between both branches of selection statement. 
We introduce intermediate values $cs_o$ and $ss_o$ and merge them in a third concurrent process.
