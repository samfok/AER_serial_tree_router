...
let's $CTRL$ reset and return before neuron vs.

\begin{hse}
*[[ci->[si];so+,co+;[~si&~ci];co-;so-
  \|si->so+;[~si];so-]]
\end{hse}

let's the neuron reset and return before $CTRL$

This differs from Rajit's probe evaluation circuit because 
\subsection{PRS}
