...
Using the sequencing in Rajit's probe evaluation circuit,
...
However, we need to wait for child to finish (\code{[$\neg$ci]}) before letting $CTRL$ proceed to next child.
This yields
...
$so$ is shared between both branches of selection statement.
Making this explicit with a third concurrent process yields
