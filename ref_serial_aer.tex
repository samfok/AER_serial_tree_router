...
\begin{easylist}
    ...
    && tree deserializer (Section~\ref{sec:DESERIAL_TREE})
    &&& \textbf{LEAF}  (Section~\ref{sec:DESERIAL_TREE_LEAF})
    &&& \textbf{NODE} (Section~\ref{sec:DESERIAL_TREE_NODE})
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Tree Deserializer \label{sec:DESERIAL_TREE}}

This design uses a tree of nodes to sequence words into their respective place
in the parallel output. That is, it takes as input e1ofN data and outputs 
eMx1ofN data. We decompose this process into LEAF and NODE.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{LEAF \label{sec:DESERIAL_TREE_LEAF}}

Leaf nodes are responsible for interfacing.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{NODE \label{sec:DESERIAL_TREE_NODE}}

try with acknowledge based sequence
\begin{hse}
*[[ x0&p0->c00+;[c0a];pa+;[~p0];x0-;x1+;c00-;[~c0a];pa-
  []x0&p1->c00+;[c0a];pa+;[~p1];x0-;x1+;c01-;[~c0a];pa-
  []x1&p0->c10+;[c1a];pa+;[~p0];x1-;x0+;c10-;[~c1a];pa-
  []x1&p1->c11+;[c1a];pa+;[~p1];x1-;x0+;c11-;[~c1a];pa-
 ]]
\end{hse}
...
%
\begin{hse}
*[[ x0&p0->c00+;[c0a];u+;pa+;[~p0];x1+;c00-;[~c0a];x0-;u-;pa-
  []x0&p1->c00+;[c0a];u+;pa+;[~p1];x1+;c01-;[~c0a];x0-;u-;pa-
  []x1&p0->c10+;[c1a];u+;pa+;[~p0];x0+;c10-;[~c1a];x1-;u-;pa-
  []x1&p1->c11+;[c1a];u+;pa+;[~p1];x0+;c11-;[~c1a];x1-;u-;pa-
 ]]
\end{hse}
...
hmm maybe do it in a separate process...
\begin{hse}
*[[c0e&c1e];pe+;u-;
    [ x0&p0->c00+;[~c0e];x0-;pe-;[~p0];c00-;x1+
    []x0&p1->c01+;[~c0e];x0-;pe-;[~p1];c01-;x1+
    []x1&p0->c10+;[~c1e];x1-;pe-;[~p0];c10-;x0+
    []x1&p1->c11+;[~c1e];x1-;pe-;[~p1];c11-;x0+
 ]]
\end{hse}

\begin{hse}
*[[c0e&c1e];pe+;
    [ pd0&p0->c00+;[~c0e];pe-;[~p0];c00-,c0d0-,c0d1-
    []pd0&p1->c01+;[~c0e];pe-;[~p1];c01-,c0d0-,c0d1-
    []pd1&p0->c10+;[~c1e];pe-;[~p0];c10-,c1d0-,c1d1-
    []pd1&p1->c11+;[~c1e];pe-;[~p1];c11-,c1d0-,c1d1-
 ]]

*[[ pd0&x0->c1d0+;[~c0e&~pd0];x1+;x0-
  []pd0&x1->c0d1+;[~c0e&~pd0];x0+;x1-
  []pd1&x0->c1d0+;[~c1e&~pd1];x1+;x0-
  []pd1&x1->c1d1+;[~c1e&~pd1];x0+;x1-
 ]]
\end{hse}
...
\begin{hse}
*[[c0e];pe+;
    [p0->c00+;[~c0e];x+;pe-;[~p0];c00-
    []p1->c01+;[~c0e];x+;pe-;[~p1];c01-];
  [c1e];pe+;
    [p0->c10+;[~c1e];x-;pe-;[~p0];c10-
    []p1->c11+;[~c1e];x-;pe-;[~p1];c11-]
 ]
\end{hse}
...
\begin{center}
    ...
    \hline \multicolumn{3}{|r|}{total transistors/deserializer} & 513 \\ \hline
    \end{tabular}
\end{center}
...
\begin{center}
    ...
    \hline \multicolumn{3}{|r|}{total transistors/deserializer} & 490 \\ \hline
    \end{tabular}
\end{center}
